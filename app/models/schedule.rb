...
class Schedule < ApplicationRecord
  ...
  def calculate_availability(scheduled_events)
    self.schedule_events.all.each do |event|
      # binding.pry
      # appointment_count = ScheduleEvent.count
      search_time = event.event_time.in_time_zone(event.schedule.time_zone)
      ...
      if found_event
        ...
        scheduled_events[found_event].availability = scheduled_events[found_event].availability - 1
      end
    end
    scheduled_events
  end
  ...
  def current_holidays
    Schedule::NATIONAL_HOLIDAYS.select { |key, _value| key.to_date.between?(Date.today, rules.inclusion.pluck(:end_date).max) }
  end
